...
export function EspStatus() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg bg-muted/30 px-3 py-1.5 text-xs">
      {!status ? (
        ...
        <>
          ...
          <span className="flex items-center gap-1.5">
            <span className={`inline-block h-1.5 w-1.5 rounded-full ${status.bt ? "bg-emerald-500" : "bg-red-500"}`} />
            <span className="text-muted-foreground">Bluetooth</span>
            <span className={`font-medium ${status.bt ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
              {status.bt ? "OK" : "NO"}
            </span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className={`inline-block h-1.5 w-1.5 rounded-full ${status.bmp1 && status.aht1 && status.mpu && status.bmp2 && status.aht2 ? "bg-emerald-500" : "bg-amber-500"}`} />
            <span className="text-muted-foreground">Sensores</span>
            <span className="font-medium text-foreground/80">
              {[status.bmp1, status.aht1, status.mpu, status.bmp2, status.aht2].filter(Boolean).length}/5
            </span>
          </span>
        </>
      )}
